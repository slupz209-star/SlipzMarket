  ...
  return (
    <div className="flex flex-col animate-fade-in-right">
      <button 
        ...
        className="w-fit flex flex-col sm:flex-row items-center gap-1.5 sm:text-xs md:text-xs lg:text-xs font-bold text-muted hover:break-words text-[#3b2a23] mb-8 transition-colors"
      >
        <ArrowLeft size={16} /> Back
      </button>
      
      <div className="w-16 md:w-18 h-18 overflow-x-auto bg-white border border-[#e5ded5] rounded-lg flex items-center justify-center mb-6 shadow-sm">
        <Mail size={16} className="text-gray-400" />
      </div>
      
      <h2 className="text-xs md:text-xs font-black break-words text-[#3b2a23] tracking-tight mb-2" style={{ fontSize: '78%' }}>Check your email</h2>
      <p className="sm:text-xs md:text-xs lg:text-xs text-gray-500 font-medium mb-8 leading-relaxed">
        We've sent a 6-digit secure code to <br/>
        <span className="font-bold break-words text-[#3b2a23]">{formData.email}</span>
      </p>

      <form onSubmit={handleVerifyOtp} className="flex flex-col gap-2">
        <div className="grid md:grid-cols-6 grid-cols-1 gap-2 sm:gap-2" onPaste={handlePaste}>
          {otp.map((digit, index) => (
            <input
              key={index}
              ...
              className="w-full h-18 sm:h-18 text-center text-xs font-black break-words text-[#3b2a23] bg-white border border-[#e5ded5] rounded-lg outline-none focus:border-[#8b6f5a] focus:ring-4 focus:ring-[#8b6f5a]/20 transition-all shadow-sm"
            />
          ))}
        </div>
        ...
        <button 
          ...
          className="w-full bg-[#8b6f5a] hover:bg-[#6c5544] text-white font-bold sm:text-xs md:text-xs lg:text-xs py-1.5 rounded-lg mt-4 shadow-xl shadow-[#8b6f5a]/20 transition-all flex items-center justify-center gap-2 disabled:opacity-70 disabled:cursor-not-allowed"
        >
          {isLoading ? <><Loader2 size={16} className="animate-spin" /> Verifying...</> : 'Verify Account'}
        </button>
      </form>

      <p className="text-center sm:text-xs md:text-xs lg:text-xs font-bold text-gray-500 mt-8">
...
const ForgotPasswordForm = ({ 
  ...
  return (
    <div className="flex flex-col animate-fade-in-right">
      ...
      <h2 className="text-xs md:text-xs font-black break-words text-[#3b2a23] tracking-tight mb-2" style={{ fontSize: '78%' }}>Reset Password</h2>
      <p className="sm:text-xs md:text-xs lg:text-xs text-gray-500 font-medium mb-8 leading-relaxed">
      ...
      <form onSubmit={handleForgotPasswordSubmit} className="flex flex-col gap-2">
        ...
        <button 
          type="submit" 
          disabled={isLoading}
          className="w-full bg-[#8b6f5a] hover:bg-[#6c5544] text-white font-bold sm:text-xs md:text-xs lg:text-xs py-1.5 rounded-lg shadow-xl shadow-[#8b6f5a]/20 transition-all flex items-center justify-center gap-2 disabled:opacity-70 disabled:cursor-not-allowed"
        >
...
const ResetPasswordForm = ({ 
  ...
  return (
    <div className="flex flex-col animate-fade-in-right">
      ...
      <h2 className="text-xs md:text-xs font-black break-words text-[#3b2a23] tracking-tight mb-2" style={{ fontSize: '78%' }}>Create New Password</h2>
      <p className="sm:text-xs md:text-xs lg:text-xs text-gray-500 font-medium mb-8 leading-relaxed">
      ...
      <form onSubmit={handleResetPasswordSubmit} className="flex flex-col gap-2">
        <div>
          <p className="sm:text-xs md:text-xs lg:text-xs font-bold break-words text-[#3b2a23] mb-3">Reset Code</p>
          <div className="grid md:grid-cols-6 grid-cols-1 gap-2 sm:gap-2" onPaste={handlePaste}>
            {otp.map((digit, index) => (
              <input
                key={index}
        ...
        <div className="flex flex-col gap-1.5 group">
          <label className="sm:text-xs md:text-sm lg:text-sm font-bold break-words text-[#3b2a23] group-focus-within:text-[#8b6f5a] transition-colors">New Password</label>
          <div className="relative">
          ...
          {newPassword.length > 0 && (
            <div className="mt-2 flex flex-col gap-1">
              <div className="flex flex-col md:flex-row gap-1.5 h-1.5 w-full">
                {[...Array(4)].map((_, i) => (
                  <div key={i} className={`flex-1 min-w-0 rounded-full transition-all duration-300 ${
                    i < passStrength 
              ...
              <p className={`text-xs font-bold ${passStrength < 2 ? 'text-red-500' : passStrength < 3 ? 'text-amber-500' : 'text-emerald-600'}`}>
        ...
        <button 
          ...
          className="w-full bg-[#8b6f5a] hover:bg-[#6c5544] text-white font-bold sm:text-xs md:text-xs lg:text-xs py-1.5 rounded-lg shadow-xl shadow-[#8b6f5a]/20 transition-all flex items-center justify-center gap-2 disabled:opacity-70 disabled:cursor-not-allowed"
        >
          {isLoading ? <><Loader2 size={16} className="animate-spin" /> Resetting...</> : <>Reset Password <ArrowRight size={16} /></>}
        </button>
      </form>
      ...
      <button 
        ...
        className="text-center sm:text-xs md:text-xs lg:text-xs font-bold text-gray-500 mt-6 hover:break-words text-[#3b2a23] transition-colors"
      >
        Back to Login
      </button>
    </div>
...
const AuthDetailsForm = ({ 
  ...
  return (
    <div className="animate-fade-in-up">
      <div className="mb-8">
        <h2 className="text-xs md:text-xs font-black break-words text-[#3b2a23] tracking-tight mb-2" style={{ fontSize: '78%' }}>
          {activeTab === 'login' ? 'Welcome back' : 'Create your account'}
        </h2>
        <p className="sm:text-xs md:text-xs lg:text-xs text-[#8b6f5a] font-medium">
          {activeTab === 'login' ? 'Access your workspace and B2B datasets.' : 'Start your 14-day free trial. No credit card required.'}
        </p>
      </div>

      <div className="overflow-x-auto bg-white p-1 rounded-lg border border-[#e5ded5] flex mb-8 shadow-sm">
        <button 
          type="button" 
          onClick={() => { setActiveTab('login'); setError(''); }} 
          className={`flex-1 min-w-0 py-1.5 text-xs font-bold rounded-lg transition-all duration-300 ${activeTab === 'login' ? 'bg-[#3b2a23] text-white shadow-sm' : 'text-[#8b6f5a] hover:break-words text-[#3b2a23]'}`}
        >
          Log In
        </button>
        <button 
          ...
          className={`flex-1 min-w-0 py-1.5 text-xs font-bold rounded-lg transition-all duration-300 ${activeTab === 'register' ? 'bg-[#3b2a23] text-white shadow-sm' : 'text-[#8b6f5a] hover:break-words text-[#3b2a23]'}`}
        >
          Sign Up
        </button>
      </div>
      ...
      <form id="auth-form" onSubmit={handleInitialSubmit} noValidate className="flex flex-col gap-3 md:gap-6">
        ...
        <div className="flex flex-col gap-1.5 group">
          <div className="flex items-center justify-between">
            <label className="sm:text-xs md:text-sm lg:text-sm font-bold break-words text-[#3b2a23] group-focus-within:text-[#8b6f5a] transition-colors">Password</label>
            {activeTab === 'login' && <button type="button" onClick={() => setIsForgotPasswordMode(true)} className="sm:text-xs md:text-xs lg:text-xs font-bold text-[#8b6f5a] hover:break-words text-[#3b2a23] transition-colors">Forgot password?</button>}
          </div>
          <div className="relative">
            <Lock size={16} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-gray-400 group-focus-within:text-[#8b6f5a] transition-colors" />
        ...
        <button 
          ...
          disabled={isLoading} 
          className="w-full bg-[#8b6f5a] hover:bg-[#6c5544] text-white font-bold sm:text-xs md:text-xs lg:text-xs py-1.5 rounded-lg mt-2 shadow-xl shadow-[#8b6f5a]/20 transition-all flex items-center justify-center gap-2 disabled:opacity-70 disabled:hover:bg-[#8b6f5a]"
        >
          {isLoading ? <><Loader2 size={16} className="animate-spin" /> Processing...</> : <>{activeTab === 'login' ? 'Log In' : 'Create Free Account'} <ArrowRight size={16} /></>}
        </button>
      </form>
      ...
      <div className="relative mt-8 mb-6">
        ...
        <div className="relative flex justify-center sm:text-xs md:text-xs lg:text-xs font-bold">
          <span className="bg-[#f5efe6] px-2 text-[#8b6f5a] tracking-widest uppercase">Or Continue With</span>
        </div>
      </div>

      <div className="flex justify-center w-full">
        <div className="rounded-lg overflow-hidden shadow-sm border border-[#e5ded5] hover:border-[#8b6f5a] transition-colors overflow-x-auto bg-white">
          <GoogleLogin 
...
const AuthComponent = () => {
  ...
  return (
    <div className="min-h-screen w-full flex flex-col lg:flex-row bg-[#f5efe6] font-sans selection:bg-[#8b6f5a] selection:text-white" style={{ zoom: '1.22' }}>
      ...
      <div className="hidden lg:flex flex-col w-full lg:w-[45%] xl:w-[45%] bg-[#3b2a23] relative overflow-hidden rounded-r-[4rem] shadow-[20px_0_50px_rgba(59,42,35,0.15)] z-10">
        ...
        <div className="absolute top-[-20%] left-[-10%] w-[600px] h-[600px] overflow-x-auto bg-white/5 rounded-full blur-3xl pointer-events-none"></div>
        <div className="absolute bottom-[-10%] right-[-20%] w-[500px] h-[500px] bg-[#8b6f5a]/20 rounded-full blur-3xl pointer-events-none"></div>

        {/* Logo */}
        <div className="absolute top-8 left-12 z-20 flex flex-col sm:flex-row items-center gap-2 cursor-pointer hover:opacity-80 transition-opacity" onClick={() => navigate('/')}>
          <Asterisk size={16} strokeWidth={3} className="text-[#d6c9b8]" />
          <span className="text-xs font-bold text-white tracking-tight" style={{ fontSize: '78%' }}>SlipZMarket</span>
        </div>

        {/* Minimal Content */}
        <div className="relative z-20 my-auto px-12 xl:px-20 flex flex-col gap-12">
          <h1 className="text-xs md:text-xs xl:text-6xl font-black text-white leading-[1.05] tracking-tight" style={{ fontSize: '78%' }}>
          ...
          <div className="flex flex-col md:flex-row gap-2 animate-fade-in-up">
            <div className="flex-1 min-w-0 overflow-x-auto bg-white/5 backdrop-blur-sm border border-white/10 rounded-lg p-6 hover:bg-white/10 transition-colors">
              <Database className="text-[#8b6f5a] mb-3" size={16} />
              <div className="text-xs font-black text-white mb-1" style={{ fontSize: '78%' }}>50M+</div>
              <div className="text-[#d6c9b8] sm:text-xs md:text-xs lg:text-xs font-medium">Verified B2B Contacts</div>
            </div>
            
            <div className="flex-1 min-w-0 overflow-x-auto bg-white/5 backdrop-blur-sm border border-white/10 rounded-lg p-6 hover:bg-white/10 transition-colors">
              <Target className="text-[#8b6f5a] mb-3" size={16} />
              <div className="text-xs font-black text-white mb-1" style={{ fontSize: '78%' }}>99%</div>
              <div className="text-[#d6c9b8] sm:text-xs md:text-xs lg:text-xs font-medium">Data Accuracy Rate</div>
            </div>
          </div>
        </div>
      </div>
      ...
      {/* RIGHT PANEL */}
      <div className="w-full lg:w-[55%] xl:w-[55%] flex min-h-screen flex-col justify-center items-center p-6 pt-10 sm:p-8 relative overflow-y-auto">
        
        {/* Mobile Header */}
        <div className="lg:hidden absolute top-6 left-6 flex flex-col sm:flex-row items-center gap-2 cursor-pointer" onClick={() => navigate('/')}>
          <Asterisk size={16} strokeWidth={3} className="text-[#8b6f5a]" />
          <span className="text-xs font-bold break-words text-[#3b2a23] tracking-tight" style={{ fontSize: '78%' }}>SlipZMarket</span>
        </div>

        <div className="w-full max-w-[420px] sm:max-w-[450px] animate-fade-in mt-6 lg:mt-0">
          
          {error && (
            <div className="mb-6 bg-red-50 border border-red-200 text-red-600 px-2 py-2 rounded-lg flex items-start gap-2 animate-fade-in-up shadow-sm">
              <AlertCircle size={16} className="mt-0.5 shrink-0" />
              <p className="sm:text-xs md:text-xs lg:text-xs font-medium leading-relaxed">{error}</p>
            </div>
          )}

          {successMsg && (
            <div className="mb-6 bg-emerald-50 border border-emerald-200 text-emerald-700 px-2 py-2 rounded-lg flex items-start gap-2 animate-fade-in-up shadow-sm">
              <CheckCircle2 size={16} className="mt-0.5 shrink-0" />
              <p className="sm:text-xs md:text-xs lg:text-xs font-medium leading-relaxed">{successMsg}</p>
            </div>
          )}
