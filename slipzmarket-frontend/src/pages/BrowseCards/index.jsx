...
const BrowseLeads = () => {
 ...
 {/* --- ENTERPRISE HEADER --- */}
 <div className="overflow-x-auto bg-white border-b border-[#d6c9b8] px-2 lg:px-3 md:px-6 py-1.5 sticky top-0 z-30 shadow-sm">
 ...
 <div>
 <h1 className="text-xs md:text-xs font-bold break-words text-[#3b2a23] tracking-tight leading-tight" style={{ fontSize: '78%' }}>{t('leadDatabase', 'Dataset Marketplace')}</h1>
 <div className="flex flex-col sm:flex-row items-center gap-2 mt-1 sm:text-xs md:text-xs lg:text-xs text-[#8b6f5a] font-medium">
 <span className="flex flex-col sm:flex-row items-center gap-1.5"><ShieldCheck size={16} className="text-emerald-600"/> Verified Network</span>
 ...
 <div className="flex items-center w-full md:w-80 bg-[#faf6f0] border border-[#d6c9b8] rounded-lg px-2 py-1.5 shadow-inner focus-within:border-[#8b6f5a] focus-within:ring-1 focus-within:ring-[#8b6f5a] transition-all">
 ...
 {/* TOASTS & MODALS */}
 {toastVisible && (
 <div className="fixed right-6 top-24 z-50 max-w-xs animate-in slide-in-from-right-8" style={{ transform: 'scale(0.9)' }}>
 <div className="rounded-lg bg-[#3b2a23] border border-[#8b6f5a] px-2 py-2 shadow-2xl text-[#faf6f0]">
 <p className="sm:text-xs md:text-xs lg:text-xs font-bold">Update</p>
 <p className="mt-0.5 sm:text-xs md:text-xs lg:text-xs leading-snug opacity-90">{toastMessage}</p>
 </div>
 </div>
 )}
 ...
 <div className="overflow-x-auto bg-white rounded-lg shadow-2xl max-w-md w-full p-7 animate-in zoom-in-95">
 <div className="w-16 md:w-20 h-20 bg-emerald-50 text-emerald-600 rounded-full flex items-center justify-center mx-auto mb-4">
 <ShoppingCart size={21} />
 </div>
 <h3 className="text-xs md:text-center text-xs font-black text-[#2a1b1b] tracking-tight" style={{ fontSize: '78%' }}>Item Added to Cart!</h3>
 <p className="text-center sm:text-xs md:text-xs lg:text-xs text-[#7a6b6b] mt-3 leading-relaxed">
 ...
 <button onClick={() => { setShowGuestAddedModal(false); window.location.href = '/cart'; }} className="w-full bg-white border-2 border-accent text-accent py-1.5 rounded-lg sm:text-xs md:text-xs lg:text-xs font-bold transition-all hover:bg-surface">
 View Cart
 </button>
 <button onClick={() => { setShowGuestAddedModal(false); window.location.href = '/auth?redirect=/cart'; }} className="w-full bg-accent text-surface py-1.5 rounded-lg sm:text-xs md:text-xs lg:text-xs font-bold transition-all shadow-xs hover:opacity-90 flex items-center justify-center gap-2">
 <LogIn size={16} /> Sign In Now
 </button>
 </div>
 <button onClick={() => setShowGuestAddedModal(false)} className="mt-4 w-full sm:text-xs md:text-xs lg:text-xs font-bold text-primary hover:text-accent py-2 transition-colors">
 Continue Shopping
 </button>
 </div>
 </div>
 ...
 <div className="flex flex-col md:flex-row gap-3 md:gap-6 px-2 lg:px-6 mt-4 md:mt-8 w-full max-w-[1400px] mx-auto items-start">
 
 {/* LEFT SIDEBAR (Tabs) */}
 <aside className="w-full md:w-40 shrink-0 bg-white border border-[#d6c9b8] rounded-lg p-4 shadow-sm md:sticky md:top-28">
 <h3 className="sm:text-xs md:text-xs md:text-sm lg:text-sm font-bold text-[#8b6f5a] uppercase tracking-widest mb-4 px-2">Data Categories</h3>
 ...
 : 'text-primary hover:bg-surface hover:text-accent hover:break-words'
 }`}
 >
 <Icon size={16} className={isSelected ? 'text-surface' : 'text-muted opacity-70'} />
 {cat.label}
 {isSelected && (
 <span className="ml-auto inline-flex h-5 min-w-[20px] px-1.5 items-center justify-center rounded-md bg-white/20 text-xs">
 ...
 {/* RIGHT MAIN CONTENT */}
 <main className="flex-1 min-w-0 w-full flex flex-col gap-2 min-w-0">
 
 {/* Top Control Bar */}
 <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between overflow-x-auto bg-white rounded-lg p-4 border border-[#d6c9b8] shadow-sm">
 <div className="flex flex-wrap gap-2 sm:text-xs md:text-xs lg:text-xs mb-4 lg:mb-0">
 <span className="inline-flex flex-col sm:flex-row items-center gap-2 px-3 py-2 rounded-lg bg-[#faf6f0] border border-[#d6c9b8]  text-[#3b2a23] font-bold">
 📊 {packageCount} total
 </span>
 <span className="inline-flex flex-col sm:flex-row items-center gap-2 px-3 py-2 rounded-lg bg-[#faf6f0] border border-[#d6c9b8]  text-[#3b2a23] font-bold">
 ...
 <div className="flex flex-col sm:flex-row items-center gap-2 border-t lg:border-t-0 border-[#d6c9b8] pt-4 lg:pt-0">
 <button
 onClick={() => setIsFilterModalOpen(true)}
 className="inline-flex flex-col sm:flex-row items-center gap-2 px-2 py-2 rounded-lg border border-theme bg-surface text-primary sm:text-xs md:text-xs lg:text-xs font-bold hover:bg-panel-soft transition-all"
 >
 <SlidersHorizontal size={16} /> Refine Data
 </button>
 <button
 onClick={selectAllPackages}
 className="inline-flex flex-col sm:flex-row items-center gap-2 px-2 py-2 rounded-lg border border-theme bg-panel-soft text-primary sm:text-xs md:text-xs lg:text-xs font-bold hover:bg-panel-strong transition-all"
 >
 ...
 {/* Bulk Action Bar */}
 <div className={`bg-accent text-surface rounded-lg p-4 flex flex-col sm:flex-row sm:items-center justify-between shadow-xl transition-all duration-300 ${selectedRows.length > 0 ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-4 hidden'}`}>
 <div className="flex flex-col sm:flex-row items-center gap-2 mb-3 sm:mb-0">
 <span className="bg-accent text-surface sm:text-xs md:text-xs lg:text-xs font-bold px-3 py-1 rounded-md">
 {selectedRows.length} Selected
 </span>
 <span className="sm:text-xs md:text-xs lg:text-xs font-medium">Bulk Value: <span className="font-bold font-sans ml-1">£{bulkTotal.toFixed(2)}</span></span>
 </div>
 <button 
 onClick={() => handleAddToCart()} 
 className="sm:text-xs md:text-xs lg:text-xs font-bold break-words text-primary bg-panel-soft hover:bg-surface px-3 md:px-6 py-1.5 rounded-lg shadow-sm transition-colors flex items-center justify-center gap-2"
 >
 ...
 {/* Bento Card Grid */}
 {isLoading ? (
 <div className="w-full py-24 flex flex-col items-center justify-center text-[#8b6f5a]">
 <Loader2 size={21} className="animate-spin mb-4" />
 <span className="sm:text-xs md:text-xs lg:text-xs font-bold">Querying Active Datasets...</span>
 </div>
 ) : filteredPackages.length === 0 ? (
 <div className="w-full py-24 text-center bg-white border border-[#d6c9b8] rounded-lg shadow-sm">
 <Database size={48} className="mx-auto text-[#d6c9b8] mb-4" />
 <span className="text-xs font-bold text-[#8b6f5a]">No datasets match your criteria.</span>
 </div>
 ) : (
 ...
 >
 {/* Select Checkbox */}
 <div className="absolute top-6 right-5 z-10" onClick={(e) => toggleRowSelection(e, pkg.id)}>
 ...
 <div>
 <h3 className="line-clamp-2 text-xs md:text-sm font-bold text-[#3b2a23] leading-tight mb-1">{pkg.brand}</h3>
 ...
 className="bg-accent text-surface p-2.5 rounded-lg transition-colors hover:opacity-90"
 >
 ...
 <div className="p-4 flex-1 min-w-0 overflow-y-auto flex flex-col gap-2">
 <div className="flex flex-col gap-2">
 <label className="sm:text-xs md:text-sm lg:text-sm font-bold text-[#8b6f5a] uppercase tracking-widest">Target Industry</label>
 <select className="w-full bg-white border border-[#d6c9b8] rounded-lg px-2 py-2 sm:text-xs md:text-xs lg:text-xs  text-[#3b2a23] font-medium outline-none focus:border-[#8b6f5a] focus:ring-1 focus:ring-[#8b6f5a]">
 ...
 <div className="flex flex-col gap-2">
 <label className="sm:text-xs md:text-sm lg:text-sm font-bold text-[#8b6f5a] uppercase tracking-widest">Job Function</label>
 <select className="w-full bg-white border border-[#d6c9b8] rounded-lg px-2 py-2 sm:text-xs md:text-xs lg:text-xs  text-[#3b2a23] font-medium outline-none focus:border-[#8b6f5a] focus:ring-1 focus:ring-[#8b6f5a]">
 ...
 <div className="p-4 border-t border-[#d6c9b8] overflow-x-auto bg-white flex flex-col md:flex-row gap-2">
 <button onClick={() => setIsFilterModalOpen(false)} className="flex-1 min-w-0 py-1.5 sm:text-xs md:text-xs lg:text-xs font-bold text-primary bg-surface border border-theme hover:bg-panel-soft transition-colors rounded-lg shadow-sm">
 Clear All
 </button>
 <button onClick={() => setIsFilterModalOpen(false)} className="flex-1 min-w-0 bg-accent text-surface py-1.5 rounded-lg sm:text-xs md:text-xs lg:text-xs font-bold shadow-sm transition-colors hover:opacity-90">
 Apply Search
 </button>
 </div>
 </div>
 ...
 <div className="relative w-full max-w-full md:w-[350px] bg-[#f5efe6] h-full shadow-2xl shadow-[#3b2a23]/20 flex flex-col animate-fade-in-right border-l border-[#d6c9b8]">
 
 {/* Drawer Header */}
 <div className="px-3 py-2 border-b border-[#d6c9b8] flex items-start justify-between overflow-x-auto bg-white">
 <div className="flex flex-col sm:flex-row items-center gap-2">
 <div className="w-14 h-14 bg-[#faf6f0] border border-[#d6c9b8] rounded-lg flex items-center justify-center shadow-sm">
 {getPackageIcon(selectedPackage.category, 20)}
 </div>
 <div>
 <h2 className="sm:text-xs md:text-xs md:text-sm lg:text-sm font-bold break-words text-[#3b2a23] leading-none mb-1.5">{selectedPackage.brand}</h2>
 ...
 <button onClick={() => setRevealPasswords(!revealPasswords)} className="sm:text-xs md:text-xs lg:text-xs font-bold text-primary bg-surface px-3 py-1 rounded-lg hover:bg-panel-soft transition-colors">
 ...
 <tr>
 <th className="px-3 md:px-6 py-1.5 sm:text-xs md:text-sm lg:text-sm font-bold text-[#8b6f5a] uppercase tracking-widest">{selectedPackage.category === 'Email & Password' ? 'Email' : 'Name'}</th>
 <th className="px-3 md:px-6 py-1.5 sm:text-xs md:text-sm lg:text-sm font-bold text-[#8b6f5a] uppercase tracking-widest">{selectedPackage.category === 'Email & Password' ? 'Password' : 'Title'}</th>
 <th className="px-3 md:px-6 py-1.5 sm:text-xs md:text-sm lg:text-sm font-bold text-[#8b6f5a] uppercase tracking-widest">Company</th>
 <th className="px-3 md:px-6 py-1.5 sm:text-xs md:text-sm lg:text-sm font-bold text-[#8b6f5a] uppercase tracking-widest">{selectedPackage.category === 'Email & Password' ? 'Access Type' : 'Contact'}</th>
 </tr>
 </thead>
