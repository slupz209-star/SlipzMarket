...
const ManageInvoices = () => {
  ...
  const getStatusBadge = (status) => {
    switch(status) {
      case 'COMPLETED': return <span className="inline-flex flex-col sm:flex-row items-center gap-1 text-xs font-bold px-2 py-1 rounded-md uppercase tracking-wider bg-emerald-50 text-emerald-700 border border-emerald-200"><CheckCircle2 size={16}/> {status}</span>;
      case 'PROCESSING': return <span className="inline-flex flex-col sm:flex-row items-center gap-1 text-xs font-bold px-2 py-1 rounded-md uppercase tracking-wider bg-amber-50 text-amber-700 border border-amber-200"><Clock size={16}/> {status}</span>;
      case 'FAILED': 
      case 'OVERDUE': return <span className="inline-flex flex-col sm:flex-row items-center gap-1 text-xs font-bold px-2 py-1 rounded-md uppercase tracking-wider bg-red-50 text-red-700 border border-red-200"><AlertTriangle size={16}/> {status}</span>;
      default: return <span className="inline-flex flex-col sm:flex-row items-center gap-1 text-xs font-bold px-2 py-1 rounded-md uppercase tracking-wider bg-[#faf6f0] text-[#8b6f5a] border border-[#d6c9b8]">{status}</span>;
    }
  };
  ...
  return (
    <div className="flex flex-col h-full min-h-screen bg-[#f5efe6] font-sans pb-12 selection:bg-[#8b6f5a] selection:text-white relative" style={{ zoom: '1.22' }}>
      ...
      {/* --- HEADER --- */}
      <div className="overflow-x-auto bg-white border-b border-[#d6c9b8] px-3 lg:px-3 py-7 sticky top-0 z-30 shadow-sm">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-2 max-w-[1400px] mx-auto w-full">
          <div>
            <h1 className="text-xs md:text-xs font-bold break-words text-[#3b2a23] tracking-tight flex flex-col sm:flex-row items-center gap-2" style={{ fontSize: '78%' }}>
              <Receipt size={21} className="text-[#8b6f5a]" /> {t('invoicesTitle')}
            </h1>
            <p className="sm:text-xs md:text-xs lg:text-xs text-[#8b6f5a] font-medium mt-0.5">{t('invoicesSubtitle')}</p>
          </div>
          <button 
            onClick={() => openDrawer()}
      ...
      <div className="px-3 lg:px-3 mt-6 max-w-[1400px] mx-auto w-full flex flex-col gap-3 md:gap-6">
        ...
        {/* Bulk Action Bar */}
        <div className={`bg-white border border-[#8b6f5a] rounded-lg p-3 flex flex-col sm:flex-row sm:items-center justify-between shadow-sm shadow-[#3b2a23]/5 transition-all duration-300 ${selectedInvoices.length > 0 ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-4 hidden'}`}>
          <div className="flex flex-col sm:flex-row items-center gap-2 mb-3 sm:mb-0 px-2">
            ...
            <span className="sm:text-xs md:text-xs lg:text-xs break-words text-[#3b2a23] font-medium">Selected Value: <span className="font-bold font-sans ml-1">£{bulkTotal.toFixed(2)}</span></span>
          </div>
          <div className="flex flex-col sm:flex-row items-center gap-2 w-full sm:w-auto">
        ...
        {/* Main Data Table */}
        <div className="overflow-x-auto bg-white border border-[#d6c9b8] rounded-lg shadow-sm overflow-hidden flex flex-col">
            ...
            <table className="w-full text-left border-collapse min-w-[900px]">
              <thead>
                <tr className="bg-[#faf6f0] border-b border-[#d6c9b8]">
                  ...
                  <th className="px-3 md:px-6 py-1.5 sm:text-xs md:text-sm lg:text-sm font-bold text-[#8b6f5a] uppercase tracking-widest">Invoice / Client</th>
                  <th className="px-3 md:px-6 py-1.5 sm:text-xs md:text-sm lg:text-sm font-bold text-[#8b6f5a] uppercase tracking-widest">Description</th>
                  <th className="px-3 md:px-6 py-1.5 sm:text-xs md:text-sm lg:text-sm font-bold text-[#8b6f5a] uppercase tracking-widest text-right">Amount</th>
                  <th className="px-3 md:px-6 py-1.5 sm:text-xs md:text-sm lg:text-sm font-bold text-[#8b6f5a] uppercase tracking-widest text-center">Status</th>
                  <th className="px-3 md:px-6 py-1.5 sm:text-xs md:text-sm lg:text-sm font-bold text-[#8b6f5a] uppercase tracking-widest text-right">Actions</th>
                </tr>
              </thead>
                ...
                {filteredInvoices.length === 0 ? (
                  <tr>
                    <td colSpan="6" className="px-3 py-12 text-center">
                      <Receipt size={16} className="mx-auto text-[#d6c9b8] mb-3" />
                      <p className="break-words text-[#3b2a23] font-bold sm:text-xs md:text-xs lg:text-xs">{t('noInvoicesFound')}</p>
                      <p className="text-[#8b6f5a] sm:text-xs md:text-xs lg:text-xs mt-1">{t('adjustSearchOrCreate')}</p>
                    </td>
                  </tr>
                ) : (
                  filteredInvoices.map((inv) => {
                    ...
                    return (
                      <tr key={inv.id} className={`transition-colors group ${isSelected ? 'bg-[#faf6f0]' : 'hover:bg-[#f5efe6]/50'}`}>
                        <td className="px-3 md:px-6 py-1.5 text-center" onClick={(e) => handleSelectRow(e, inv.id)}>
                          <input 
                            type="checkbox" 
                            checked={isSelected}
                            onChange={() => {}}
                        ...
                        <td className="px-3 md:px-6 py-1.5">
                          <div className="flex flex-col">
                            <span className="sm:text-xs md:text-xs lg:text-xs font-bold break-words text-[#3b2a23] leading-tight">{inv.id}</span>
                            <span className="sm:text-xs md:text-xs lg:text-xs font-medium text-[#8b6f5a] flex flex-col sm:flex-row items-center gap-1 mt-0.5"><Building2 size={16} /> {inv.client}</span>
                          </div>
                        </td>
                        <td className="px-3 md:px-6 py-1.5">
                          <div className="flex flex-col">
                            <span className="sm:text-xs md:text-xs lg:text-xs font-medium break-words text-[#3b2a23] leading-tight">{inv.description}</span>
                            <span className="sm:text-xs md:text-xs lg:text-xs text-[#8b6f5a] mt-0.5 flex flex-col sm:flex-row items-center gap-1"><Calendar size={16}/> Billed: {inv.date}</span>
                          </div>
                        </td>
                        <td className="px-3 md:px-6 py-1.5 text-right">
                          <span className="sm:text-xs md:text-xs lg:text-xs font-sans font-bold break-words text-[#3b2a23]">£{inv.amount.toFixed(2)}</span>
                        </td>
                        ...
                        <td className="px-3 md:px-6 py-1.5 text-right">
                          <div className="flex items-center justify-end gap-1 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity">
                            <button onClick={() => openDrawer(inv)} className="p-1.5 text-[#8b6f5a] hover:bg-[#faf6f0] hover:break-words text-[#3b2a23] rounded-md transition-colors border border-transparent hover:border-[#d6c9b8]" title="Edit">
                              <Edit2 size={16} />
                            </button>
          ...
          <div className="px-3 py-1.5 border-t border-[#d6c9b8] bg-[#faf6f0] flex items-center justify-between">
            <span className="sm:text-xs md:text-xs lg:text-xs font-bold text-[#8b6f5a] uppercase tracking-widest">{filteredInvoices.length} {t('invoicesTotal')}</span>
          </div>
        </div>
          ...
          <div className="relative w-full max-w-[450px] bg-[#f5efe6] h-full shadow-2xl shadow-[#3b2a23]/20 flex flex-col animate-fade-in-right border-l border-[#d6c9b8]">
            <div className="px-3 py-7 border-b border-[#d6c9b8] overflow-x-auto bg-white flex items-center justify-between shrink-0">
              <h3 className="text-xs md:text-xs font-bold break-words text-[#3b2a23] flex flex-col sm:flex-row items-center gap-2">
                <Edit2 size={16} className="text-[#8b6f5a]"/> {editingInvoice ? 'Edit Invoice' : 'Create Invoice'}
              </h3>
            ...
            <form id="invoice-form" onSubmit={handleSaveInvoice} className="p-6 flex-1 min-w-0 overflow-y-auto flex flex-col gap-3 md:gap-6">
              ...
              <div className="flex flex-col gap-1.5">
                <label className="sm:text-xs md:text-sm lg:text-sm font-bold text-[#8b6f5a] uppercase tracking-widest">Client / Organization</label>
                <input 
                  type="text" required 
              ...
              <div className="flex flex-col gap-1.5">
                <label className="sm:text-xs md:text-sm lg:text-sm font-bold text-[#8b6f5a] uppercase tracking-widest">Line Item / Description</label>
                <textarea 
                  required rows="2"
              ...
              <div className="grid md:grid-cols-2 grid-cols-1 gap-2">
                <div className="flex flex-col gap-1.5">
                  <label className="sm:text-xs md:text-sm lg:text-sm font-bold text-[#8b6f5a] uppercase tracking-widest">Amount (£)</label>
                  <div className="relative">
                    <DollarSign size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-[#8b6f5a]" />
                    <input 
                ...
                <div className="flex flex-col gap-1.5">
                  <label className="sm:text-xs md:text-sm lg:text-sm font-bold text-[#8b6f5a] uppercase tracking-widest">Billing Date</label>
                  <input 
                    type="date" required 
                    value={formData.date} 
              ...
              <div className="flex flex-col gap-1.5">
                <label className="sm:text-xs md:text-sm lg:text-sm font-bold text-[#8b6f5a] uppercase tracking-widest">Payment Status</label>
                <select 
